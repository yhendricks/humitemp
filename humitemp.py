...
# cycle_1_enabled = True
# cycle_2_enabled = True
# max_temperature_1 = 50
# max_temperature_2 = 50
# max_humidity_1 = 95
# max_humidity_2 = 70
current_cycle_duration_in_minutes = 1440  # minutes
# cycle_2_duration_in_minutes = 1440  # minutes
current_cycle_start_time = datetime.now()  # tbd
current_cycle_index = 0
# cycle_2_start_time = datetime.now()  # tbd
...
class CycleConfig:
    ...
    max_temperature = 50
    max_humidity = 95
    cycle_duration_in_minutes = 1440
    cycle_start_time = datetime.now()


def measure_and_control():
    global last_temperature, last_humidity, cycle_started, current_cycle_duration_in_minutes, current_cycle_start_time, current_cycle_index
    ...
    for index, cycle in enumerate(cycle_config):
        ...
        if not preheat_started:
            ...
            app_log.info('cycle {} started on {} with duration of {}'
                         .format(index, current_cycle_start_time.now().strftime("%Y-%m-%d %H:%M:%S"), cycle.cycle_duration_in_minutes ))

        while cycle_started or preheat_started:
            current_cycle_duration_in_minutes = cycle.cycle_duration_in_minutes
            if cycle_started and datetime.now() > cycle.cycle_start_time + timedelta(minutes=cycle.cycle_duration_in_minutes):
...
def display_state():
    ...
    if cycle_started or preheat_started:
        ...
        if cycle_started:
            ...
            print 'Cycle State:'
            print '============'
            print '     Cycle {} started on {}'.format(current_cycle_index, current_cycle_start_time.strftime("%Y-%m-%d %H:%M:%S"))
            print '     Cycle running for {:.0f} minutes with {:.2f}% completed '.format(minutes, perc)
        else:
            print '==================='
...
def set_gpio_pin():
    try:
        ...
        if pin >= 0 and pin <= 17:  # up to 17 gpio pins on pcduino3
...
def measure_temperature():
    ...
    return (fudge_avg_val / STEPS_PER_DEGREE) - TEMPERATURE_OFFSET, avg_val, fudge_avg_val    # returns 0 - 4095  for a range of 0V to 3.3
...
def measure_humidity():
    ...
    return (fudge_avg_val / STEPS_PER_DEGREE), avg_val, fudge_avg_val,     # returns 0 - 4095  for a range of 0V to 3.3
...
if __name__ == "__main__":
    #init()          # this is required for colorama on windows
    # setup logging
    ...
    try:
        cycle1 = CycleConfig()
        cycle1.max_temperature = config.getint('configuration0', 'max_temperature')
        cycle1.max_humidity = config.getint('configuration0', 'max_humidity')
        cycle1.cycle_duration_in_minutes = config.getint('configuration0', 'cycle_duration_in_minutes')
        cycle1.cycle_enabled = True
        cycle_config.append(cycle1)
    except:
        app_log.info("Cycle 0 configuration error")

    try:
        cycle2 = CycleConfig()
        cycle2.max_temperature = config.getint('configuration1', 'max_temperature')
        cycle2.max_humidity = config.getint('configuration1', 'max_humidity')
        cycle2.cycle_duration_in_minutes = config.getint('configuration1', 'cycle_duration_in_minutes')
        cycle2.enabled = True
        cycle_config.append(cycle2)
    except:
...
# # http://git.agocontrol.com/apagg/agocontrol-work/blob/93f7d9051aebf79c794e3d5fa4feea92c9221831/raspiMCP3xxxGPIO/MCP3208.py
#
